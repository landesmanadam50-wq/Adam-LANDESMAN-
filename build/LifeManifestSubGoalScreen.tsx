 ...
 * screen -- the 5 ARC Goal actions (spec section 1), dates + optional
 * deadline reminders (section 3), its Targets (section 4-6), its
 * journal (section 3/5), and the Sub-goal completion flow (sections 2/9).
 * Kept off the questionnaire wizard (LifeManifestEditorScreen.tsx) to
 * avoid overloading it -- reached via "ניהול תת־המטרה" from there, or
 * directly from a notification/journal entry/dashboard button. Reached
 * by the Sub-goal's own globally-unique id alone (no LifeManifest id in
 * the route) via findSubGoalOwner -- see arc/lifeManifest.ts's own doc.
 */
export default function LifeManifestSubGoalScreen() {
  ...
  /** Sub-goal↔ARC Goal connection task, spec section 7's "ליצור ARC Goal חדש" offered from a Target with no ARC Goal of its own -- creates a real ArcGoal (never a duplicate), sets THIS Target (not the Sub-goal) to arcGoalLinkMode "own" pointing at it, and opens it. */
  ...
  if (status === "notFound" || !owner) {
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.content}>
          <Text style={styles.title}>תת־המטרה לא נמצאה</Text>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.eyebrow}>{`${majorGoal.title} · תת־מטרה ${order}`}</Text>
        <Text style={styles.title}>{subGoal.title || "תת־מטרה ללא כותרת"}</Text>
        ...
        <View style={[styles.chipColumn, styles.chipRow]}>
          {COMPLETION_MODES.map((mode) => (
            ...
            >
              <Text style={styles.buttonText}>{COMPLETION_MODE_LABELS[mode]}</Text>
            </Pressable>
          ))}
        </View>
      ...
      <View style={[styles.chipColumn, styles.chipRow]}>
        {TARGET_STATUSES.map((s) => (
          <Pressable key={s} style={[styles.chip, target.status === s && styles.chipSelected]} onPress={() => onPatch({ status: s })}>
            <Text style={styles.buttonText}>{TARGET_STATUS_LABELS[s]}</Text>
          </Pressable>
        ))}
      </View>
